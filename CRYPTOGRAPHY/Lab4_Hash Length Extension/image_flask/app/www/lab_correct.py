...
def verify_mac(key, my_name, uid, cmd, download, mac):
    """
    Updated function: Uses HMAC-SHA256 instead of simple concatenation.
    This effectively mitigates Length Extension Attacks.
    """
    # 1. Construct the message string exactly as the client does
    download_message = '' if not download else '&download=' + download
    message = ''
    if my_name:
        message = 'myname={}&'.format(my_name)
    
    # The message is formed by concatenating arguments (without the secret key)
    message += 'uid={}&lstcmd='.format(uid) + cmd + download_message
    
    # 2. Compute the HMAC-SHA256
    # Instead of hashlib.sha256(key + ":" + message), we use the hmac module.
    # The hmac.new() function applies the key in a two-pass process:
    ...
    # Logging for debugging purposes (check the terminal logs of the container)
    app.logger.debug('Message to verify: [{}]'.format(message))
    app.logger.debug('Computed HMAC: [{}]'.format(real_mac))
    
    # 3. Securely compare the provided MAC with the computed HMAC
    # hmac.compare_digest is used to prevent timing attacks by ensuring 
    # the comparison takes a constant amount of time regardless of match position.
    return hmac.compare_digest(real_mac, mac)
