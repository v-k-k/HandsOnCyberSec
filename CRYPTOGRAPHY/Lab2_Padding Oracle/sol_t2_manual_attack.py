...
if __name__ == "__main__":
    ...
    # searching for D2[15]
    for K in range(1, 7):
        print(f"\n--- searching byte for K={K} (D2[{16-K}]) ---")
        
        # 1. preparing CC1: preparing found bytes for new padding value (K)
        # if K=3, last 2 bytes should XOR with D2 to meet 0x03
        for j in range(1, K):
            CC1[16 - j] = D2[16 - j] ^ K
        
        # 2. going through 256 variants for current byte
        found = False
        for i in range(256):
            CC1[16 - K] = i
            status = oracle.decrypt(IV + CC1 + C2)
            ...
            if status == "Valid":
                print(f"found valid value: i = 0x{i:02x}")
                # 3. calculating mid byte D2
                D2[16 - K] = i ^ K
                print(f"D2[{16-K}] = 0x{D2[16-K]:02x}")
                found = True
                break
        
        if not found:
            print(f"error: couldn`t find value for K={K}")
            break
    ...
    print("mid state of D2 (last 6 bytes): " + D2[10:16].hex())
    print("P2:  " + P2.hex())
    
     # print only decyphered bytes (last 6)
    print("decyphered P2 tail: ", P2[10:16])
    print("="*30)
